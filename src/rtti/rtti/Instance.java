 ...
 * 当对某个对象进行强制类型转换的时候，可以先用instanceof判断正确性。
 * 
 * @author 刘晨伟
 * 
 * 创建日期：2007-8-6
...
public class Instance {
	...
	static void test(Object x) {
		/*
		 * instanceof和isInstance()生成结果完全相同
		 */
		...
		System.out.println("x instance of MyInterface " + (x instanceof MyInterface));
		System.out.println("Base.isInstance(x) " + Base.class.isInstance(x));
		System.out.println("Derived.isInstance(x) " + Derived.class.isInstance(x));
		System.out.println("MyInterface.isInstance(x) " + MyInterface.class.isInstance(x));

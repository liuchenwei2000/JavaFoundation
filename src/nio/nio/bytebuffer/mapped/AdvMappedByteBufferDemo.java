...
public class AdvMappedByteBufferDemo {
	...
	/**
	 * 本例通过计算压缩文件CRC32校验和来证明
	 * 内存映射文件比传统输入流、缓冲输入流都要快
	 * 
	 * @param args
	 */
...
class MappedFileCRCSumChecker implements ICRCSumChecker {
	...
	public long checkCRCSum(String filePath) throws Exception {
		...
		CRC32 crc = new CRC32();
		/*
		 * 通过调用FileChannel类的map方法可以获得MappedByteBuffer
		 * 可以指定需要进行映射的文件区域和映射模式。有三种模式：
		 * 1，FileChannel.MapMode.READ_ONLY：结果缓冲区是只读的
		 * 任何试图写入缓冲区的举动都将依法抛出ReadOnlyBufferException
		 * 2，FileChannel.MapMode.READ_WRITE：结果缓冲区可写，这些改变会及时写回文件中
		 * 3，FileChannel.MapMode.PRIVATE：结果缓冲区可写
		 * 但是任何改变都是私有的，仅仅对该缓冲区有效，并不会写入文件中
		 * 
		 * map方法将文件的一部分映射到内存中
		 */

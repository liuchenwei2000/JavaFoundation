...
/**
 * ByteBuffer所有的视图缓冲器演示
 * <p>
 * 一旦底层的ByteBuffer通过视图缓冲器被填满整数或其他基本类型时，
 * 就可以直接被写到通道中了，正像从通道中读取那样容易。
 * 然后使用视图缓冲器可以把任何数据都转化成某一特定的基本类型。
 * 在本例中，通过在同一个ByteBuffer上建立不同的视图缓冲器，
 * 将同一字节序列翻译成了short,int,float,long,和double类型的数据。
 * 
 * @author 刘晨伟
 *
 * 创建日期：2009-7-23
...
public class ViewBuffers {
	...
	public static void main(String[] args) {
		...
		byteBuffer.rewind();
		/*
		 * ByteBuffer是一个被"包装"过的8字节数组
		 * 然后通过各种不同的基本类型的视图缓冲器把它显示了出来
		 * 可以在输出中看到，当从不同类型的缓冲器读取时，数据显示的方式也不同
		 */

 ...
 * transient关键字演示类
 * <p>
 * 当对序列化进行控制时，可能存在某个特定字段不想让 Java 的序列化机制自动保存与恢复。
 * 如果字段表示的是不希望将其序列化的敏感信息(如密码)，通常就会面临这种情况，
 * 即使对象中的这些信息是 private 属性，一经序列化处理就可以通过读取文件或者拦截网络传输的方式来访问到它。
 * <p>
 * 有一种防止对象的敏感部分被序列化的办法，就是将类实现为 Externalizable。
 * 这样就没有任何东西可以自动序列化，并且可以在 writeExternal() 内部只对所需部分进行显式的序列化。
 * <p>
 * 然而如果正在操作的是一个 Serializable 对象，那么所有序列化操作都会自动进行。
 * 为了能够予以控制，可以用 transient 关键字逐个字段地关闭序列化。
 * <p>
 * 由于 Externalizable 对象在缺省情况下不保存它们的任何字段，
